...
- **If `linked_items.last_error` shows `there is no unique or exclusion constraint matching the ON CONFLICT specification`:** the original `schema_v3.sql` created partial unique indexes on `transactions.external_id` and `holdings.external_id`, which PostgREST's upsert (`onConflict: "external_id"`) can't infer. Run [`supabase/schema_v3_fix_upsert_indexes.sql`](supabase/schema_v3_fix_upsert_indexes.sql) once in the Supabase SQL editor — it rebuilds them as full unique indexes and resets any `status='error'` rows. Symptom: bank link succeeds, sync immediately fails, Money tab shows no transactions.
